...
public class Ssq {
    ...
    private static void analyzerAllMin() {
        ...
        String line = String.format("\n正在分析，样本：%s->%s 共 %s 期 ，出现次数最多的球）...", list.get(0).get("v"), list.get(1).get("v"), list.get(2).get("v"));
        LOGGER.info(line);
        FileUtils.write2File(path, line);

        List<String> adviceRed = new ArrayList<String>();
        List<String> adviceBlue = new ArrayList<String>();
        List<Map> reds = DBFactory.getDb().findAll("select * from (\n" +
                "select code,count(*) tme from ssq_detail where `order`<7 group by code \n" +
                ") t order by t.tme asc limit 0,6");

        for (Map map : reds) {
            line = String.format("红球，号码:%s,次数：%s", map.get("code"), map.get("tme"));
            LOGGER.info(line);
            FileUtils.write2File(path, line);
            adviceRed.add(map.get("code").toString());
        }
        List<Map> blues = DBFactory.getDb().findAll(" select * from (\n" +
                "                select code,count(*) tme from ssq_detail where `order`=7 group by code \n" +
                ") t order by t.tme asc  limit 0,3");

        for (Map map : blues) {
            line = String.format("蓝球，号码:%s,次数：%s", map.get("code"), map.get("tme"));
            LOGGER.info(line);
            FileUtils.write2File(path, line);
            adviceBlue.add(map.get("code").toString());
        }

        line = String.format("根据出现最少频率，建议买入号码：%s", Objects.join(",", adviceRed) + "+" + Objects.join(",", adviceBlue));
        LOGGER.info(line);
        FileUtils.write2File(path, line);
    }

    private static void analyzerAllMax() {
        ...
        List<Map> reds = DBFactory.getDb().findAll("select * from (\n" +
                "select code,count(*) tme from ssq_detail where `order`<7 group by code \n" +
                ") t order by t.tme DESC limit 0,6");
        ...
        List<Map> blues = DBFactory.getDb().findAll(" select * from (\n" +
                "                select code,count(*) tme from ssq_detail where `order`=7 group by code \n" +
                ") t order by t.tme desc  limit 0,3");
        ...
        line = String.format("根据出现最多频率，建议买入号码：%s", Objects.join(",", adviceRed) + "+" + Objects.join(",", adviceBlue));
        LOGGER.info(line);
        FileUtils.write2File(path, line);
    }

    public static void analyzerOrderMax() {
        ...
        String line = String.format("\n正在分析，样本：%s->%s 共 %s 期 ，各个序号出现频率最多的号码如下（前" + LIMIT + "）...", list.get(0).get("v"), list.get(1).get("v"), list.get(2).get("v"));
        LOGGER.info(line);
        FileUtils.write2File(path, line);
...
//          }
        ...
        line = String.format("根据各个序号出现的最多频率，建议买入号码：%s", Objects.join(",", advice) + "+" + blue);
        LOGGER.info(line);
        FileUtils.write2File(path, line);
    }


    public static void analyzerOrderMin() {
        ...
        String line = String.format("\n正在分析，样本：%s->%s 共 %s 期 ，各个序号出现频率最少的号码如下（前" + LIMIT + "）...", list.get(0).get("v"), list.get(1).get("v"), list.get(2).get("v"));
        LOGGER.info(line);
        FileUtils.write2File(path, line);

        String sql = "select * from (\n" +
                "\tselect t1.order,t1.code,min(time) time  from (\n" +
                "\t select `order`,`code`,count(*) time from ssq_detail where `order` =1 group by `order`,`code` \n" +
                "\t) t1  group by t1.order,t1.code,t1.time order by t1.time asc limit 0," + LIMIT + "\n" +
                ") t1\n" +
                "union\n" +
                "select * from (\n" +
                "\tselect t1.order,t1.code,min(time) time  from (\n" +
...
//          }
        ...
        String blue = advice.remove(6);
        line = String.format("根据各个序号出现的最少频率，建议买入号码：%s", Objects.join(",", advice) + "+" + blue);
        LOGGER.info(line);
        FileUtils.write2File(path, line);
    }
